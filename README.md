...
├── Android/                         # Android 앱
│   ├── build.gradle.kts             # Android 프로젝트 빌드 설정
│   ├── settings.gradle.kts          # Gradle 프로젝트 설정
│   ├── local.properties             # BASE_URL 등 로컬 실행 설정
│   └── app/
│       ├── build.gradle.kts         # 앱 모듈 빌드 설정
│       └── src/main/
│           ├── AndroidManifest.xml  # 권한, Activity, 앱 설정
│           ├── java/com/example/moodtender/
│           │   ├── MainActivity.kt        # 페어링 여부 확인 후 화면 분기
│           │   ├── LoginActivity.kt       # 로그인 진입 Activity
│           │   ├── LoginScreen.kt         # 로그인 Compose UI
│           │   ├── ChatScreen.kt          # AI 바텐더 채팅 화면
│           │   ├── HealthScreen.kt        # 건강 데이터 확인 화면
│           │   ├── PairingScreen.kt       # PIN 기반 기기 연동 화면
│           │   ├── PermissionScreen.kt    # 사용 기록/권한 안내 화면
│           │   ├── ApiService.kt          # Retrofit API 인터페이스
│           │   ├── RetrofitClient.kt      # 서버 통신 클라이언트
│           │   ├── HealthDataWorker.kt    # Health Connect 기반 데이터 수집
│           │   ├── data/
│           │   │   ├── Models.kt          # 채팅/LLM/건강 응답 모델
│           │   │   ├── HealthDataRequest.kt # 건강 데이터 전송 요청 모델
│           │   │   └── UserCreate.kt      # 로그인 요청 모델
│           │   ├── workers/
│           │   │   └── DataSyncWorker.kt  # 주기적 서버 동기화 Worker
│           │   └── ui/theme/              # Compose 테마 설정
│           └── res/
│               ├── drawable/              # 배경/이미지 리소스
│               ├── raw/                   # Lottie 등 raw 리소스
│               ├── values/                # 색상, 문자열, 테마
│               └── xml/                   # 백업/데이터 추출 규칙
├── frontend/                        # 웹 화면
├── run.ps1                          # 서버 실행 스크립트
...
Android 서버 주소는 아래 두 방식 중 프로젝트 코드에 맞는 방식으로 설정합니다.

현재 프로젝트 방식:

```text
Android/local.properties
BASE_URL=http://개인_IP:포트/
```

`RetrofitClient.kt`에서 직접 지정하는 방식으로 사용할 경우:

```kotlin
private const val BASE_URL = "http://개인_IP:포트/"
```

실제 휴대폰에서 테스트할 때는 `127.0.0.1`이 아니라 노트북/PC의 Wi-Fi IP를 넣습니다.
