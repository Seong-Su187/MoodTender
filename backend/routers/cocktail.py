...
from backend.services.monthly_service import generate_language_change_analysis
...
async def get_weekly_ai_report(
    ...
    # 최근 7일 감정 영수증에서 채팅 기반 감정 추출
    seven_days_ago = datetime.utcnow() - timedelta(days=7)
    emotion_rows = await db.execute(
        select(UserMemory.sub_category)
        .where(UserMemory.user_id == user_id)
        .where(UserMemory.sub_category != None)
        .where(UserMemory.sub_category != "")
        .where(UserMemory.created_at >= seven_days_ago)
        .order_by(UserMemory.created_at.desc())
    )
    emotions = [r[0] for r in emotion_rows.fetchall()]

    if emotions:
        primary_emotion = Counter(emotions).most_common(1)[0][0]
        if metrics_result.get("status") == "success":
            metrics_result["analysis"]["emotion"] = primary_emotion
        else:
            metrics_result = {
                "status": "success",
                "analysis": {"emotion": primary_emotion, "reason": "대화 기반 감정 분석"},
                "deltas": {"steps": 0, "sleep": 0, "screen": 0},
                "baseline": {}, "today": {}
            }
